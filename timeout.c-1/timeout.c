...
/* 1. FUNÇÃO settimeout:
   Esta função configura o cronômetro do sistema para disparar um sinal após a duração especificada. 
   Ela tenta usar cronômetros de alta precisão se disponíveis, caso contrário, usa o alarme padrão. */
...
/* 2. FUNÇÃO send_sig:
   Envia um sinal de sistema para o processo ou grupo alvo. 
   Se o alvo for o grupo, ela faz o processo atual ignorar o sinal para evitar loops de sinalização. */
...
/* 3. FUNÇÃO chld:
   Tratador de sinal vazio para SIGCHLD. É necessário para que a função sigsuspend() 
   possa ser interrompida quando o processo filho terminar. */
...
/* 4. FUNÇÃO cleanup:
   Realiza a limpeza quando ocorre um timeout ou o programa é interrompido. 
   Ela envia os sinais de término para o processo monitorado e gerencia a opção de matar após o tempo. */
static void
cleanup (int sig)
{
  if (sig == SIGALRM)
...
/* 5. FUNÇÃO usage:
   Exibe as informações de uso do comando timeout, descrevendo as opções disponíveis e os códigos de saída. */
void
usage (int status)
{
...
/* 6. FUNÇÃO apply_time_suffix:
   Aplica um multiplicador a um valor de ponto flutuante com base em um caractere de sufixo (segundos, minutos, horas, dias). */
...
/* 7. FUNÇÃO parse_duration:
   Analisa a string de duração fornecida na entrada, validando o número e aplicando os sufixos de tempo necessários. */
static double
...
/* 8. FUNÇÃO unblock_signal:
   Desbloqueia um sinal específico na máscara de sinais do processo atual. */
static void
...
/* 9. FUNÇÃO install_sigchld:
   Instala o tratador de sinal para SIGCHLD para monitorar quando os processos filhos terminam. */
static void
...
/* 10. FUNÇÃO sig_needs_handling:
    Determina se um sinal específico precisa ser tratado ou se pode ser ignorado. */
...
/* 11. FUNÇÃO install_cleanup:
    Registra a função cleanup como tratadora para diversos sinais de término de sistema. */
static void
...
/* 12. FUNÇÃO block_cleanup_and_chld:
    Bloqueia sinais de limpeza e o sinal SIGCHLD para evitar condições de corrida durante a espera do processo. */
...
/* 13. FUNÇÃO disable_core_dumps:
    Tenta desabilitar a criação de despejos de memória (core dumps) para o processo atual. */
...
/* 14. FUNÇÃO main:
    Função principal que coordena a execução do comando, gerencia o fork do processo filho e o monitoramento do tempo. */
int
